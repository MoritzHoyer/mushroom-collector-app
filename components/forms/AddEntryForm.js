...
export default function AddEntryForm({ onMutate }) {
    ...
    useEffect(() => {
        if (typeof window !== "undefined" && navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
                ...
                (error) => {
                    console.error(
                        "Error when retrieving location data:",
                        error
                    );
                }
            );
    ...
    const validateInputs = () => {
        ...
        if (!name.trim())
            validationErrors.name = "Please enter a suspected name.";
    ...
    const handleImageChange = (e) => {
        ...
        if (!allowedFormats.includes(file.type)) {
            setErrors({
                image: "Invalid file format. Please upload an image in JPG, PNG, WEBP, HEIC or HEIF format.",
            });
            return;
        }
